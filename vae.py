...
class VAE(nn.Module):
    def __init__(self,latent_dim=32):
        ...
        self.decoder_input = nn.Linear(latent_dim, 256 * 4 * 4) #linear layer to transform latent space back to the shape of the last conv layer output

        self.decoder = nn.Sequential(
            nn.Unflatten(1, (256, 4, 4)),  # reshape the flat vector into a grid   (256, 4, 4) giving us 256 stack of 4x4 feature maps

            #the bottom layers upsample the image backto the original size with 1 feature map (in channels, out channels )
